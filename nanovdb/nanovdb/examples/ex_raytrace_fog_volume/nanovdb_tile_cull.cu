...
//
// Single-kernel design: each thread (= one pixel in a 16x16 tile) tests
// its ray against the tree's upper-internal-node bboxes inline, then
// either short-circuits to background (no upper hit) or runs the fixed-
// step transmittance integration on a ray clipped to the union of hit-
// upper t-ranges.
//
// Versus the level-set version, only the per-pixel work after the
// upper-internal cull differs (fog integration instead of ZeroCrossing).
                             ...
                             float* __restrict__              outImage)
{
    ...
    // upper-internal bbox and accumulate (tMin, tMax).  If no upper is hit
    // we short-circuit to background; otherwise we clip the ray to the
    // union and run the fog integration below.
    ...
    if (tMax < tMin) {
        // empty pixel -- write background (alpha=0).
                        ...
                        int width, int height, BufferT& imageBuffer)
{
    ...
    std::cout << "Tile-cull fog tracer avg ms: " << (totalTime / numIterations) << "\n";
