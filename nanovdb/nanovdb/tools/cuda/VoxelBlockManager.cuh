...
// Capability macros: consumer-matched cached-leaf stencil resolvers are available -
// computeCrossStencil (7-tap Laplacian-class) and computeBoxStencilCached (27-tap random
// access). Partial-tap consumers (e.g. 19-tap WENO) keep the naive/DCE box path: the
// shootout found a right-sized edge table regresses on dense grids and small block widths.
#define NANOVDB_VBM_HAS_CACHED_BOX 1
#define NANOVDB_VBM_HAS_CROSS_STENCIL 1
...
struct VoxelBlockManager : nanovdb::tools::VoxelBlockManagerBase<Log2BlockWidth>
{
    ...
    ///
    /// Use this for consumers that need random access to all 27 materialized taps: it beats the
    /// naive per-tap resolution on sparse grids (measured -13..-34%). For consumers that only
    /// reduce/accumulate taps, prefer forEachBoxStencil (streaming) - it is faster still and
    /// occupancy-robust, whereas this cached path can regress on very dense grids at small
    /// block widths.
    /// @tparam BuildT Build type of the grid
